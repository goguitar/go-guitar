...
func spawn_chord(notes: Array, time: float, chord_name: String = "") -> Node3D:
	...
	var left_x := Highway.fret_separator_world_x(min_fret - 1)
	var right_x := Highway.fret_separator_world_x(min_fret + BORDER_FRET_SPAN - 1)
	var center_x := (left_x + right_x) * 0.5
	var top_y := Highway.string_world_y(0)
	var bottom_y := Highway.string_world_y(5) - Highway.STRING_SLOT_HEIGHT
	...
	if frame:
		...
		for note_data in notes:
			...
			var x := Highway.fret_mid_world_x(f - 1) - center_x
			var y := Highway.string_world_y(s) - center_y
