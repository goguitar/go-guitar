...
func _create_fretboard() -> void:
	...
	fretboard.size = Vector3(Common.FRET_WORLD_WIDTH + 2, 0.1, 100)
	fretboard.position = Vector3(Common.FRET_WORLD_WIDTH * 0.5, -0.05, 50)
	...
	for fret in range(1, FRET_COUNT + 1):
		var fret_x := Common.fret_separator_world_x(fret)
...
func _create_strings() -> void:
	for str_idx in range(STRING_COUNT):
		var str_y := Common.string_world_y(str_idx)
		var str := CSGBox3D.new()
		str.name = "String_%d" % str_idx
		str.size = Vector3(Common.FRET_WORLD_WIDTH + 2, 0.015, 0.015)
		str.position = Vector3(Common.FRET_WORLD_WIDTH * 0.5, str_y, 4)
		var str_mat := StandardMaterial3D.new()
		str_mat.albedo_color = Common.STRING_COLORS[str_idx]
...
func set_lane_intensities(values: Array[float]) -> void:
	...
	for i in range(mini(values.size(), STRING_COUNT)):
		var str_mesh: CSGBox3D = _string_meshes[i] if i < _string_meshes.size() else null
		if str_mesh:
			var base_color := Common.STRING_COLORS[i]
...
func set_active_fret_range(min_fret: int, max_fret: int) -> void:
	...
	for i in range(_fret_markers.size()):
		var fret := i + 1
		var marker: CSGBox3D = _fret_markers[i] if i < _fret_markers.size() else null
		if marker:
